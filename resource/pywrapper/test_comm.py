...
class YourExistingApp(QMainWindow):
    def __init__(self):
        ...
        self.ultrasound_backend.provider_updated.connect(self.on_provider_received)
        ...
        btn = QPushButton("测试调用 request_provider")
    ...
    def process_ultrasound_frame(self, image_matrix):
        try:
            ...
            print(f"PyQt 显示转换失败: {exc}")

    def on_device_state_changed(self, state):
        pass

    def check_provider_data(self):
        self.ultrasound_backend.request_provider()

    def on_provider_received(self, data_dict):
        print(data_dict)
